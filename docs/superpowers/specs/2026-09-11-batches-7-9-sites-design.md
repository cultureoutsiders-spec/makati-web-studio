...
| 2 | Quicker than Quick | Makati mall branch (confirm exact unit off FB) | FB DM `/quickerthanquickph` | "the four-counter" — one kiosk, four trades: shoe & bag repair, key duplication, locksmithing, custom small leather. Tabbed service board (inline JS tabs), while-you-wait vs leave-it split, quote form. |
